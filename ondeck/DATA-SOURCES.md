...
// ondeck/data/sirens-fixtures.json
{
  ...
  "opponents": {
    // Per-opponent logo map keyed by `opponentShort`. The fetcher reads
    // `g.opponentLogo ?? opponents[opponentShort] ?? null`, so individual
    // games can still override. Centralizing here avoids repeating the URL
    // on every game vs the same opponent (5 of the Sirens' 7 opponents
    // play home & away). Self-host under assets/opponents/ — see the
    // "Opponent logos for static-fixture teams" section below.
    "Stockton Cargo SC": "assets/opponents/stockton-cargo.png",
    "Marin FC":          "assets/opponents/marin-fc.png"
    // ...
  },
  "games": [
    {
    ...
    // optional per-game fields: opponentLogo (overrides the opponents map),
    // broadcasts: []
  ]
}
```
...
`pastDays`/`futureDays` window ESPN games use, resolves `opponentLogo`
via per-game override then the `opponents` map, and merges everything
into `games.json` with stable IDs like `mb-sirens-static-0`.
...
fixtures file triggers a `games.json` refresh on push. (Adding new files
under `ondeck/assets/opponents/` does *not* trigger a refresh, because the
existing `games.json` already references those paths — GitHub Pages just
starts serving the new asset on its next deploy.)

## Opponent logos for static-fixture teams

Self-hosted under `ondeck/assets/opponents/{slug}.png`, referenced from
the `opponents` map in the fixtures file by relative path. This was
the conclusion after a quick survey for the Sirens; the dead ends are
worth recording.

**Footgun**: the USL W League hosts opponent logos on
`vip.sportsengine.com/usl/wp-content/uploads/sites/119/2023/10/{team}-logo.png`.
A HEAD on these returns 200 with a `content-type: image/png` — but the
served PNG is **25×25**, an autogenerated favicon thumbnail. There is no
size query parameter that surfaces a larger version; the original upload
is not retained at full resolution on that CDN. Don't reuse these URLs.

**Sources that worked, per team** (USL W NorCal, 2026 season):

| Opponent          | Source                                          |
|-------------------|-------------------------------------------------|
| Stockton Cargo SC | `stocktonsc.com` site favicon (`cropped-MicrosoftTeams-image-4.png`) |
| Marin FC          | `marinfc.org` apple-touch-icon (`Marin-FC-Crest-18-scaled.png`)      |
| Oakland Soul SC   | `oaklandsoulsc.com` brand asset (`oakland-soul@3x.png`)              |
| Pleasanton RAGE   | `pleasantonrage.org` favicon-192 (`RageLogo2019.png`)                |
| San Juan SC       | `cdn1.sportngin.com` USL W team-page asset                           |
| California Storm  | Wikipedia (`upload.wikimedia.org/...California_Storm_FC_Logo.png`)   |

The pattern that worked across all six: parse the `<link rel="icon">`
and `<link rel="apple-touch-icon">` from each team's WordPress site for
the highest-resolution version; the `apple-touch-icon` is typically the
team crest at usable size. For teams whose primary site is a JS-redirect
shell (California Storm), check Wikipedia or the league's team page.

Downscale to ~256px max with `sips -Z 256 file.png --out file.png`. The
Sirens' six opponent logos total **280 KB** at that resolution, which is
fine for a personal site. The render target is 36×36 on the card
(`.card-logo`), so 256px gives roughly 7× retina headroom.
